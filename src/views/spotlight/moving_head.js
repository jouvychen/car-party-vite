...
var position_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES * 3), 3).setUsage(THREE.DynamicDrawUsage);
var direction_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES * 3), 3).setUsage(THREE.DynamicDrawUsage);
var intensity_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES), 1).setUsage(THREE.DynamicDrawUsage);
var color_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES * 3), 3).setUsage(THREE.DynamicDrawUsage);
var emissive_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES), 1).setUsage(THREE.DynamicDrawUsage);
var angle_buffer_attribute = new THREE.InstancedBufferAttribute(new Float32Array(MAX_INSTANCES * 2), 2).setUsage(THREE.DynamicDrawUsage);
...
class MovingHead {
  ...
  constructor(data = {
    ...
    this.prepareInstance(data.mainScene);
   ...
   */
  updateMatrix() {
    if (this._matrixNeedsUpdate && baseMesh instanceof THREE.Object3D) {
  ...
  controlIntensity() {
    ...
    const leftDoorDeg = new TWEEN.Tween(shutter);
    leftDoorDeg.to(
      {
        test: 0,
      },
    ...
    leftDoorDeg.onUpdate(function () {
      _this._spotLight.intensity = SPOTLIGHT_PHYSICALLY_CORRECT_INTENSITY * _this.intensity * shutter.test;
      intensity_buffer_attribute.setX(_this._id, _this.intensity * shutter.test);
      intensity_buffer_attribute.needsUpdate = true;
    });
    // leftDoorDeg.onComplete(() => {
    //   leftDoorDeg.start();
    // });
    leftDoorDeg.start();
  }
  ...
  controlIntensityYoyo() {
    ...
    new TWEEN.Tween({ y: initialY })
      .to({ y: targetY }, 3000)
      .onUpdate(({y}) => {
        this._spotLight.intensity = SPOTLIGHT_PHYSICALLY_CORRECT_INTENSITY * this.intensity * y;
      intensity_buffer_attribute.setX(this._id, this.intensity * y);
      intensity_buffer_attribute.needsUpdate = true;
      })
      .yoyo(true) // 设置yoyo效果
      .repeat(Infinity) // 无限循环播放
      .start(); // 启动动画
  }
  ...
  static async prepareModelInstance(scene) {
    const loader = new GLTFLoader();
    let gltf = await loader.loadAsync('/visualizer/models/scenography/moving_head_lowpoly.glb');
  ...
  static prepareCapInstance() {
    ...
    capMesh = new THREE.InstancedMesh(targetGeo, capMaterial, 100);

    capMesh.instanceCount = 10;
  ...
  static prepareInstanciation(camera, scene) {
    ...
    scene_handle = scene;
    MovingHead.prepareModelInstance(scene);
    MovingHead.prepareBeamInstance(scene);
    MovingHead.prepareCapInstance();
    MovingHead.prepareBoxHelperInstance();

    scene.add(beamMesh, capMesh, boxHelperMesh);
  }
